...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if all(pos in self.s_levels[level] for pos in a_node.prenodes):
                for s_node in self.s_levels[level]:
                    for pre_s_node in a_node.prenodes:
                        if pre_s_node.__eq__(s_node):
                            s_node.children.add(a_node)
                            a_node.parents.add(s_node)

                self.a_levels[level].append(a_node)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
            for eff in a_node.effnodes:
                ...
                self.s_levels[level].append(s_node)

        self.s_levels[level] = list(set(self.s_levels[level]))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        pos_effects_1 = {eff.symbol for eff in effects_1 if eff.is_pos}
        neg_effects_1 = {eff.symbol for eff in effects_1 if not eff.is_pos}
        pos_effects_2 = {eff.symbol for eff in effects_2 if eff.is_pos}
        neg_effects_2 = {eff.symbol for eff in effects_2 if not eff.is_pos}

        # If there is an intersection between the add and remove effects then it is effects mutex
        if pos_effects_1.intersection(neg_effects_2) or pos_effects_2.intersection(neg_effects_1):
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effects_1 = node_a1.effnodes
        prenodes_1 = node_a1.prenodes
        effects_2 = node_a2.effnodes
        prenodes_2 = node_a2.prenodes

        pos_effects_1 = {eff.symbol for eff in effects_1 if eff.is_pos}
        neg_effects_1 = {eff.symbol for eff in effects_1 if not eff.is_pos}
        precond_pos_1 = {pre.symbol for pre in effects_1 if pre.is_pos}
        precond_neg_1 = {pre.symbol for pre in effects_1 if not pre.is_pos}

        pos_effects_2 = {eff.symbol for eff in effects_2 if eff.is_pos}
        neg_effects_2 = {eff.symbol for eff in effects_2 if not eff.is_pos}
        precond_pos_2 = {pre.symbol for pre in effects_2 if pre.is_pos}
        precond_neg_2 = {pre.symbol for pre in effects_2 if not pre.is_pos}

        if any([                                      \
            pos_effects_1.intersection(precond_neg_2),\
            pos_effects_2.intersection(precond_neg_1),\
            neg_effects_1.intersection(precond_pos_2),\
            neg_effects_2.intersection(precond_pos_1) \
            ]):
            return True
        else:
            return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if any(p1.is_mutex(p2) for p1 in parent_1 for p2 in parent_2):
            return True
        else:
            return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if not node_s1.__eq__(node_s2) and node_s1.symbol == node_s2.symbol:
            return True
        else:
            return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        if (parent_1 or parent_2) and not any(not s1.is_mutex(s2) for s1 in parent_1 for s2 in parent_2):
            return True
        else:
            return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for i, level in enumerate(self.s_levels):
                for node in level:
                    if node.__eq__(PgNode_s(goal, True)):
                        found = True
                if not found:    
                    index += 1
            level_sum += index
